import  re
import copy
from ..src.color_builder import Colors, ColorBuilder, ColorBuildenPlainText
import numpy as np
...
class Component:
    ...
    def take_step(self, key, term, rest):
        def update_clock_receive(sender_clk, component_clk):
            k = list(sender_clk.keys())
            sc = np.array(list(sender_clk.values()))
            cc = np.array(list(component_clk.values()))
            n = np.where(sc > cc, sc, cc)
            d = dict(zip(k, n))
            return d
